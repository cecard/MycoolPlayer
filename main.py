...
class ArtGenerator:
    ...
    def draw_icon(size=64):
        ...
        painter.setBrush(QBrush(QColor(BG_SIDE)))
        ...
        painter.setPen(pen)
        w, h = size, size
        painter.drawLine(int(w*0.35), int(h*0.7), int(w*0.35), int(h*0.3))
        painter.drawLine(int(w*0.65), int(h*0.6), int(w*0.65), int(h*0.2))
        painter.drawLine(int(w*0.35), int(h*0.3), int(w*0.65), int(h*0.2))
        painter.end()
        return QIcon(pixmap)
...
class ModernPlayer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("全能音乐播放器 Pro")
        ...
        self.player.errorOccurred.connect(self.handle_errors) # 错误监听
    ...
    def init_ui(self):
        ...
        self.btn_switch_mode = QPushButton("🛠️ 歌词工坊")
        ...
        page_play = QWidget()
        play_layout = QHBoxLayout(page_play)
        ...
        self.lbl_song_title.setStyleSheet("font-size: 22px; font-weight: bold; margin-top: 20px;")
        ...
        self.txt_maker.setPlaceholderText("粘贴歌词 -> 点击录制 -> 按空格键...")
        ...
        self.btn_record = QPushButton("🎙️ 录制 (空格)")
        ...
        maker_layout.addWidget(QLabel("歌词制作模式"))
    ...
    def select_folder(self):
        ...
        if folder:
            ...
            if self.playlist:
                ...
                QMessageBox.information(self, "提示", "该文件夹下没有找到支持的音频文件。")
    ...
    def play_music(self, path):
        ...
        for img in ['cover.jpg', 'folder.jpg', 'cover.png', 'folder.png']:
            if os.path.exists(os.path.join(folder, img)):
                self.lbl_cover.setPixmap(QPixmap(os.path.join(folder, img)))
                cover_found = True
                break
        if not cover_found:
            self.lbl_cover.setPixmap(ArtGenerator.draw_default_cover(320))
        ...
        self.load_lrc(path)
        if self.is_maker_active: self.toggle_maker_record()
    ...
    def handle_errors(self):
        self.btn_play.setText("▶")
        self.lbl_song_title.setText("播放错误：不支持的格式")
        # 自动切歌防止卡死
        QMessageBox.warning(self, "播放失败", f"无法播放此文件:\n{self.player.errorString()}\n可能需要安装系统解码器。")

    def handle_media_status(self, status):
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            if self.play_mode == 1: self.player.play()
    ...
    def load_lrc(self, audio_path):
        ...
        self.lbl_lrc_pre.clear(); self.lbl_lrc_next.clear()
    ...
    def toggle_main_view(self):
        if self.stack.currentIndex() == 0:
            ...
            self.btn_switch_mode.setText("🛠️ 歌词工坊")

    def toggle_maker_record(self):
        if self.btn_record.isChecked():
            text = self.txt_maker.toPlainText().strip()
            if not text:
                self.btn_record.setChecked(False); QMessageBox.warning(self, "提示", "请先输入歌词文本"); return
            self.maker_lines = [l.strip() for l in text.split('\n') if l.strip()]
            self.maker_timestamps = []; self.maker_current_idx = 0; self.is_maker_active = True
            self.lbl_maker_status.setText(f"录制中... 下一句: {self.maker_lines[0]}")
            self.btn_record.setText("⏹ 停止 (点击结束)"); self.setFocus()
        else:
            self.is_maker_active = False; self.lbl_maker_status.setText("录制已停止"); self.btn_record.setText("🎙️ 录制 (空格)")
    ...
    def keyPressEvent(self, event):
        if self.is_maker_active and event.key() == Qt.Key.Key_Space:
            if self.maker_current_idx < len(self.maker_lines):
                ...
                else: self.lbl_maker_status.setText("录制完成")
            else: self.toggle_maker_record()
        ...
        try:
            with open(path, 'w', encoding='utf-8') as f:
                for i in range(min(len(self.maker_timestamps), len(self.maker_lines))):
                    ...
                    f.write(f"[{ms//60000:02}:{(ms%60000)/1000:05.2f}]{self.maker_lines[i]}\n")
            QMessageBox.information(self, "成功", f"歌词已保存至: {path}"); self.load_lrc(self.playlist[self.current_index])
